...
import com.huyhieu.widget.databinding.WidgetUButtonBinding
...
class UButtonView @JvmOverloads constructor(
    ...
    private fun setText(name: String?) {
        with(binding) {
            if (!name.isNullOrEmpty()) {
                contentType = com.huyhieu.widget.commons.UButtonView.CONTENT_TYPE_TEXT
                tvName.text = name
            }
            if (radiusValue == 0F) {
    ...
    private fun setIcon(name: String?, drawable: Drawable?) {
        with(binding) {
            if (name.isNullOrEmpty()) {
                contentType = com.huyhieu.widget.commons.UButtonView.CONTENT_TYPE_ICON
            }
            if (drawable != null) {
    ...
    private fun setUIParent(
        ...
        this.setCardBackgroundColor(
            when {
                ...
                !isEnabled -> context.color(com.huyhieu.library.R.color.colorGrayscaleDisable)
                else -> bgColor
            }
        )
